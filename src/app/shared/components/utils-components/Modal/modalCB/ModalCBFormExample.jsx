...
const ModalCBForm = ({ submit, errorLog }) => (


    
    <form  onsubmit="return false"  onSubmit={(values)=>{alert(JSON.stringify(values))}}class="flex flex-wrap gap-3 w-full p-5">
      ...
      <label class="relative flex-1 flex flex-col">
       ...
       min="2020-08" value="2020-10"/>        <svg xmlns="http://www.w3.org/2000/svg" class="absolute bottom-0 left-0 -mb-0.5 transform translate-x-1/2 -translate-y-1/2 text-black peer-placeholder-shown:text-gray-300 h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className='flex justify-end self-end m-2 text-sm	'>
                        <button className="paiementCart h-12 w-24" >Pay</button>
                    </div>
    </form>
)
 ...
 * @author Luca Toscana
 */
const ModalCBFormComponent = (props) => {
...
export default ModalCBFormComponent;